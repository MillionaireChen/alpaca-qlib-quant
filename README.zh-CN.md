...
一套端到端、可复现的量化股票系统:在 NASDAQ-100 成分股上训练截面收益排序模型,
把排序结果转化为 Top-K 组合,在真实交易成本下衡量其表现,
然后用**同一套**策略逻辑通过 Alpaca **Paper Trading**(模拟账户)API 实盘执行。

纸面交易是默认且唯一开放的执行模式——实盘端点是在代码层面被硬性拦截的,
不是靠"约定俗成"来避免。

## 这是个什么项目

多数"量化策略"代码到一条回测曲线就结束了。问题在于,回测是金融里最容易被无意造假的东西:
一处标签错位、一次在未来数据上做的归一化、一个零成本假设,
就能让一个根本不赚钱的想法看起来很漂亮。本仓库反过来设计——**正确性与诚实的度量第一,收益第二**:

* 所有结果都由已提交的脚本在真实历史数据上跑出、含交易成本、可从配置快照完整复现;
* 所有泄漏陷阱被显式处理并写入文档(标签对齐、执行位移、带 embargo 的时序切分、
  归一化不含未来信息、逐日时点成分股);
* 回测的策略逻辑与纸面交易执行的是同一套,因此两者可以直接对比;
* 交易层把操作安全的优先级放在收益之上——无效信号直接拒绝交易、
  订单提交前全量校验、绝不假设成交、券商状态永远是唯一事实来源。

它是一个研究与纸面交易平台,不是赚钱承诺。实测出的超额收益
(见[已验证结果](#6-已验证结果))真实但很薄、且依赖市场状态,本文档对此直说不粉饰。
...
```
市场数据(日线 OHLCV,复权,逐日时点股票池)
   ↓  scripts/prepare_data.py       — 下载并验证 qlib 美股数据包
特征工程(Alpha158,157 个特征,+ 10 个自定义因子)
   ↓  core/handlers.py, factors/    — qlib 表达式与 pandas 双实现,交叉验证一致
数据集构建(时序切分、embargo、前视收益标签)
   ↓  core/handlers.py
LightGBM 训练(以验证集 Rank IC 早停)
   ↓  scripts/train.py              — 模型 + 完整复现元数据
收益预测与信号质量分析(逐日 IC / Rank IC / 分层收益)
   ↓  scripts/predict.py
截面排序 → Top-K 组合(等权,换手受限)
   ↓  strategies/topk.py
含成本回测 + 绩效报告(9 项指标,4 张图)
   ↓  scripts/backtest.py
Walk-forward 评估(5 个滚动窗口,逐窗重新训练)
   ↓  scripts/walk_forward.py
纸面交易:目标组合 → 差分订单 → 风控校验 → Alpaca paper
   ↓  scripts/paper_trade.py        — 永远先跑 dry-run
订单/持仓/PnL 监控、对账、回测与实盘对比
      scripts/account_status.py, reconcile.py, paper_vs_backtest.py
```

具体交付物:9 个命令行入口、与券商解耦的交易层(含 Alpaca 适配器与内存 mock 券商)、
10 个自定义因子(pandas/qlib 双实现)、纯函数指标库、时间戳实验追踪,
以及 61 项单元测试(覆盖因子正确性、无未来函数保证、指标数学、全部交易安全路径)。
...
| 决策 | 理由 |
|---|---|
| 标签 `close[t+5]/close[t+1] − 1`,t+1 收盘建仓 | 信号在 t 日收盘后形成,因此绝不可能用计算特征时已知的价格成交 |
| 以验证集 **Rank IC** 早停,而非 MSE | 选股是排序问题。用 MSE 时模型在这个约 100 只股票的截面上第 1 轮就停(信号低于 L2 噪声下界);Rank IC 才能选出真有排序能力的模型 |
| train/valid 段末加 `embargo_days: 10` | 5 日前视标签需要最多 6 个未来交易日;不加 embargo,训练标签会与验证期重叠 |
| 逐日时点 `nasdaq100` 成分股 | 用今天的成分股回测 2016 年,等于把幸存者偏差直接烤进结果里 |
| 成本恒定开启(单边 5bp + 每笔最低 $1) | 对一个日均换手 0.40 的策略做零成本回测,那不是结果,是假象 |
| 以 walk-forward 作为对外口径 | 单段切分容易变成对特定时期的过拟合;5 个重训窗口才能看出超额在不同市场状态下的真实表现 |
| 订单只来自目标与当前持仓的**差分** | 每日重建组合会换掉 100% 仓位;差分只动真正变化的部分 |
| 券商状态为准,成交必须轮询 | 本地期望会与现实脱节(部分成交、拒单、过期挂单);对账要把差异暴露出来,而不是掩盖 |
| 实盘在代码层拦截 | 一个配置笔误不应该有能力把真金白银发出去 |

研究者会想调的一切——股票池、基准、预测期、切分、模型超参、K 值、换手上限、
成本、风控阈值——全部在 `configs/` 下的 YAML 里,不在 Python 代码里。

## 当前状态

已在本仓库用真实数据端到端验证:数据准备、特征工程、训练、预测/IC 分析、
Top-K 回测、绩效报告、自定义因子、walk-forward 评估。
纸面交易层(券商抽象、订单生成、风控、dry-run、执行、对账、对比)已实现,
并已用 mock 券商完成验证;唯一剩下的是对 Alpaca 的**首次真实纸面会话**,
这一步必须在你自己的机器上跑——见[首次里程碑](#73-首次里程碑在-mac-上按序执行)。

已知限制:内置数据集止于 2020-11,所以实盘纸面交易前需要先刷新数据
(见[未来扩展](#12-未来扩展)第一项);在刷新之前,信号过期保护会主动拒绝交易。
...
└── README.md / README.zh-CN.md / CLAUDE.md   # 英文文档 / 本文档 / 项目工程规范
```
